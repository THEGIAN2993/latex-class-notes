...
From a categorical perspective, now that we have studied measure spaces, we turn our attention to the maps between them.
...
\end{proposition}
    {\color{red} \begin{proof}
        Come back, something about the homework 1 problem
    \end{proof}}
...
\begin{proposition}\label{prop:limits-of-measurable-functions-are-measurable}
    Let $(X,\cM)$ be a measurable space and $(f_j)_{j = 1}^\infty$ be a sequence of functions in $\overline{\bfR}^X$ which are $\cM$-measurable for each $j \in \bfN$. The functions:
        \begin{equation*}
        \begin{split}
            \sup_j f_j, \h9 \inf_j f_j, \h9 \limsup_j f_j, \h9\liminf_j f_j
        \end{split}
        \end{equation*}
...
Recall that the Riemann Integral involves partitioning the domain of our function into disjoint intervals, and looking at the distance of these step functions and multiplying it by their height. {\color{red} explain intuiviely what we want the lebesgue integral to do.}

For the remainder of this section, fix a measure space $(X,\cM,\mu)$.
...
{\color{red} ( motivate simple functions as a way to talk about integration???)}
...
\begin{theorem}\label{thm:increasing-sequence-of-simple-functions}
    Let $f:X \rightarrow [0,\infty]$ be $\cM$-measurable. There exists a sequence of simple functions $(\varphi_n)_n$ such that $0 \leq \varphi_1 \leq \varphi_2 \leq... \leq f$ and $\varphi_n \rightarrow f$. If $f$ is bounded, then this convergence is uniform.
\end{theorem}
    {\color{red} \begin{proof}
        :( 10/15
    \end{proof}}

The above proof is hard to read, but it is easily understood graphically:

{\color{red} include graphics here :(}

\begin{definition}
    \phantom{a}
    \begin{enumerate}[label = (\arabic*),itemsep=1pt,topsep=3pt]
        \item The space of all measurable functions from $X$ to $[0,\infty]$ is denoted:
            \begin{equation*}
            \begin{split}
                L^+ := \{f:X \rightarrow [0,\infty] \mid f \text{ is }\cM\text{-measurable }\}
            \end{split}
            \end{equation*}
        \item The subset of $L^+$ which contains only simple functions is denoted:
            \begin{equation*}
            \begin{split}
                S^+ := \{\varphi:X \rightarrow [0,\infty] \mid \varphi \in L^+, \h1 \varphi\text{ is a simple function }\}.
            \end{split}
            \end{equation*}
    \end{enumerate}
\end{definition}

Let $\varphi = \sum_{j = 1}^n a_j \chi_{E_j}$ be a simple function in standard representation. Intuitively, the area under this curve should be $\sum_{j = 1}^n a_j \mu(E_j)$ Moreover, the area under the curve of any measurable function $f$ should be the "best" approximation of area under a simple function strictly contained between $f$ and $0$. {\color{red} This sounds fucking terrible}
...
We will often abuse notation in the following way: if $(X,\cM,\mu)$ is a measure space and $f \in L^+$, then:
    \begin{equation*}
    \begin{split}
        \int f :=: \int f d\mu :=: \int_X f d\mu :=: \int_X f(x) d\mu(x)
    \end{split}
    \end{equation*}
are all equivalent ways of saying the "integral of $f$". We will mostly be taking the minimalistic approach of $\int f$ when the measure space we are working over is clear. {\color{red} sounds bad}
...
In order to prove the equivalent (c) and (d) of Proposition~\ref{}, we need extra machinery.

\begin{proposition}\label{prop:int-equals-zero-iff-funct-equals-zero}
    Let $f \in L^+$. Then $\int f = 0$ if and only if $f = 0$ a.e.
\end{proposition}
    \begin{proof}
        ($\Leftarrow$) Define $E:= \{x \mid f(x) = 0\}$. Then by assumption $\mu(E^c) = 0$. Let $\varphi \in S^+$ and $\varphi \leq f$. Write $\varphi = \sum_{i = 1}^n a_i \chi_{E_i}$ in standard representation. Since $\varphi \leq f$...



        Define $E := \{x \mid f(x) = 0\}$ and $N := E^c = \{x \mid f(x) \neq 0\}$. ($\Leftarrow$)  Suppose $\mu(N) = 0$. Let $\varphi \in S^+$ and $0 \leq \varphi \leq f$. Suppose $\varphi = \sum_{i = 1}^n a_i \chi_{E_i}$ is in standard representation. Since $\varphi \leq f$, if $a_i > 0$ then it must be the case that $E_i \subset N${\color{red}*} (we know that $\varphi \leq f$, where $f = 0$ a.e., so if $a_i > 0 $, it must be the case that $\mu(E_i) = 0$ ). Hence $\mu(E_i) = 0$. But since $\int \varphi = \sum_{i = 1}^n a_i \mu(E_i) = 0$, it must be the case that $\sup\{\int \varphi \mid 0 \leq \varphi \leq f, \varphi \in S^+\} = 0$.
        
        
        ($\Rightarrow$) Now suppose $\int f = 0$. Let $E_n = \{x \mid f(x) \geq \frac{1}{n}\}$. Then $E_1 \subset E_2 \subset E_3 \subset ...$, and $P := \bigcup_{i = 1}^\infty E_i = \{c \mid f(x) > 0 \}$. If $\mu(P) = 0$, then we are done. If $\mu(P) > 0$, then since $\mu(P) = \lim_{n \rightarrow \infty}\mu(E_n)$, there must exist some $n \in \bfN$ such that $\mu(E_n) > 0$. It follows that:
            \begin{equation*}
            \begin{split}
                \int f 
                & \geq \int_{E_n} f \\
                & = \int f \chi_{E_n} \\
                & \geq \int \frac{1}{n} \chi_{E_n} \\
                & = \frac{1}{n} \mu(E_n) \\
                & > 0.
            \end{split}
            \end{equation*}
        This is a contradiction, so it must be the case that $\mu(P) = 0$. Hence $f = 0$ a.e.
    \end{proof}
...
\end{theorem}
    \begin{proof}
        We will prove this by showing inequality in both directions. Note that for all $n \in \bfN$ we have $f_n \leq f$. It must be the case that if $\varphi \in S^+$ such that $\varphi \leq f_n$, then $\varphi \leq f$ as well. We've obtained the set inclusion:
            \begin{equation*}
            \begin{split}
        ...
        Hence:
            ...
            \begin{split}
                \int f 
                & = \sup \{ \ts \int \varphi \mid \varphi \leq f, \varphi \in S^+\} \\
                & \geq \sup \{ \ts \int \varphi \mid \varphi \leq f_n, \varphi \in S^+\} \\
                & = \int f_n.
            \end{split}
            \end{equation*}
        Taking $n \rightarrow \infty$ gives $\int f \geq \lim_{n \rightarrow \infty} \int f_n$.

        Now fix $\varphi \in S^+$ such that $0 \leq \varphi \leq f$, and fix a constant $0 < \alpha < 1$. Define $E_n := \{x \mid f_n(x)\geq \alpha \varphi(x)\}$. We can see that:
            \begin{equation*}
            \begin{split}
                E_n 
            ...
            \end{split}
            \end{equation*}
        Since $f_n$ are $\varphi$ are $\cM$-measurable functions, there difference $f_n - \alpha \varphi$ is $\cM$-measurable, hence $E_n \in \cM$. In particular, since $(f_n)_n$ is an increasing sequence, we have $E_1 \subset E_2 \subset E_3 \subset ...$ and $\bigcup_{i = 1}^\infty E_i = X${\color{red}*($(f_n)_n$ is an increasing sequence so surely $f_{n+1} \geq f_n(x) \geq \alpha \varphi(x)$, hence $E_{n+1} \supset E_n$. also, $\varphi \leq f$, and since $\alpha \in (0,1)$, we have $ \alpha \varphi < f$. but this is true for all $x$, and $f_n \rightarrow f$, hence the unions of all the $E$'s should be our entire set.)}. Now note that for all $x \in X$ we have $\alpha \varphi \chi_{E_n} \leq f_n$, which gives:
            \begin{equation*}
            \begin{split}
                \int f_n 
                & \geq \int \alpha \varphi \chi_{E_n} \\
                & = \alpha \int_{E_n} \varphi.
            \end{split}
            \end{equation*}
        But recall that $A \mapsto \int_A \varphi$ defines a measure on $(X,\cM)$! Applying continuity from below gives:
            \begin{equation*}
            \begin{split}
                \alpha \int \varphi 
                & = \lim_{n \rightarrow \infty} \alpha \int_{E_n} \varphi \\
                & \leq \lim_{n \rightarrow \infty} \int f_n.
            \end{split}
            \end{equation*}
        Sending $\alpha \rightarrow 1$ gives $\int \varphi \leq \lim_{n \rightarrow \infty} \int f_n$. Taking the supremum over all $\varphi \in S^+$ such that $\varphi \leq f$ gives $\int f \leq \lim_{n \rightarrow \infty}\int f_n$.
    \end{proof}
...
We can use the Monotone Convergence Theorem to prove a more general result on increasing sequence of functions which converge almost everywhere.
...
\end{theorem}
    \begin{proof}
        For each $k \in \bfN$, define $g_n := \inf_{k \geq n}f_k $. Then $g_n$ is measurable by Proposition~\ref{prop:limits-of-measurable-functions-are-measurable}, and in particular $g_n \nearrow \liminf_{n \rightarrow \infty} f_n$ (since the limit infimums form an increasing sequence). Note that $g_n \leq f_k$ for all $k \geq n$, hence $\int g_n \leq \int f_k$ for all $k \geq n$. Since $\int g_n$ is a lowerbound for the set $\left\{\int f_k \mid k \geq n\right\}$, it must be the case that $\int g_n \leq \inf_{k \geq n} \int f_k$. By the Monotone Convergence Theorem:
            \begin{equation*}
            \begin{split}
...
\begin{corollary}
    Let $(f_n)_n$ be a sequence in $L^+$. Suppose $f_n \rightarrow f$ a.e. Then $\int f \leq \lim_{n \rightarrow \infty}\int f_n$.
\end{corollary}
    \begin{proof}
        {\color{red} sketch: let $E = \{ x \mid f_n(x) \rightarrow f(x)\}$. Replace $f$ w/ $f \chi_E$ and $f_n$ w/ $f_n \chi_e$. apply fatous lemma i think.}
    \end{proof}
...
\end{proposition}
    {\color{red} not sure how to do this rigorously \begin{proof}
...
\begin{theorem}[Dominated Convergence Theorem]
    Let $(f_n)_n$ be a sequence in $L^1$ and suppose $f_n \rightarrow f$ a.e. for some $f:X \rightarrow \bfC$. Moreover, suppose there exists $g \in L^1$ such that $g \geq 0$ and for all $n \in \bfN$, we have $|f_n| \leq g$ a.e. Then (after possibly changing $f$ on a null set), $f \in L^1$ and $\int f = \lim_{n \rightarrow \infty} \int f_n$.
\end{theorem}
    \begin{proof}
        ...
        Hence:
            ...
            \end{split}
            \end{equation*}
        Thus $f \chi_E \in L^1$. Now write $f_n \chi_E = u_n +i v_n$ and $f \chi_E = u + iv$, where $u_n \rightarrow u$ and $v_n \rightarrow v$. Surely $g \pm u_n \geq 0$ and $g \pm v_n \geq 0$, hence by Fatou's Lemma:
            \begin{equation*}
            \begin{split}
        ...
        Since $u_n \rightarrow u$ we have $\liminf_{n \rightarrow \infty} u_n = \lim_{n \rightarrow \infty}u_n = u$, giving:
            ...
            \end{split}
            \end{equation*}
        Subtracting $\int g$ from both sides gives $\int u \leq \liminf_{n \rightarrow \infty \int u_n}$. Now applying Fatou's Lemma to $g - u_n$ gives:
            \begin{equation*}
            \begin{split}
                \int \liminf_{n \rightarrow \infty}(g - u_n) \leq \liminf_{n \rightarrow \infty} \int (g - u_n).
            \end{split}
...
\end{theorem}
    \begin{proof}
        ...
        We have that $\sum_{n = 1}^\infty f_n = f$ a.e. Likewise:
            ...
            \end{split}
            \end{equation*}
        Hence $\left| \sum_{j = 1}^n f_j \right| \leq g \chi_E$ a.e., and $\sum_{j = 1}^n f_j \rightarrow f$ a.e. By the Dominated Convergence Theorem, $f \in L^1$ and $\int f = \lim_{n \rightarrow \infty} \int \sum_{j = 1}^n f_n$. Finally:
            \begin{equation*}
            \begin{split}
...
\end{theorem}
    \begin{proof}
        ...
        Note that $g_P \leq f \leq G_P$ holds for any partition $P$. Choose a sequence of partitions $P_1 \subset P_2 \subset P_3 \subset...$ such that:
            ...
            \end{split}
            \end{equation*}
        Let $G := \lim_{k \rightarrow \infty}G_{P_k}$ and $g := \lim_{k \rightarrow \infty}g_{P_k}$. Note that $g$ and $G$ are measurable since they are limits of measurable functions. Moreover, by construction $g \leq f \leq G$, hence $G - g \in L^+$. Applying the Dominated Convergence Theorem gives:
            \begin{equation*}
            \begin{split}
...
{\color{red} missed a lot of stuff}
