...
\nameref{thm:premeasure} will finally allow us to construct measures on the real number line. As we showed in \S 2.0, we are unable to define measures on the power set of $\bfR$. Our game-plan is to find an algebra $\cA$ of subsets of $\bfR$ such that, after applying \nameref{thm:premeasure}, we obtain a measure on the Borel sets of $\bfR$.
    ...
    \end{theorem}
        \begin{proof}
            (1)$\Rightarrow$(2) Let $E \in \cM_\mu$. By Theorem~\ref{thm:measurable-sets-prop}, there exists open sets $\{U_n\}_{n \in \bfN}$ such that:
                ...
                \end{split}
                \end{equation*}
            Since $\mu(E) < \infty$, we can write this as $\mu(U_n)  - \mu(E) < \frac{1}{2^n}$. Define $V = \bigcap_{n \in \bfN}U_n$. Then $V$ is a $G_\delta$ set. Since $E \subset U_n$ for all $n \in \bfN$, we have $E \subset V$. By monotonicity, we have $\mu(E) \leq \mu(V)$. But we also have:
                \begin{equation*}
                \begin{split}
                    \mu(V) \leq \mu(U_n) \leq \mu(E) + \frac{1}{2^n}.
                \end{split}
                \end{equation*}
            Taking $\epsilon \rightarrow 0$ gives $\mu(V) \leq \mu(E)$. Together, $\mu(V) = \mu(E)$; i.e., $\mu(V \setminus E) = 0$. Define $N_1 = V \setminus E$. Then $E = V \setminus N_1$, where $V$ is a $G_\delta$ set and $N_1$ is a null-set.
    ...
    \begin{definition}
        The Lebesgue-Stieltjes measure on $\bfR$ associated to the distribution function $F(x) = x$ is called the \textit{Lebesgue measure on $\bfR$}, and is denoted by $m$. The domain of $m$ is the collection of $\mu^\ast$-measurable sets, which we will denote as $\cL$.
    \end{definition}
    ...
    \end{theorem}
        \begin{proof}
            Every open set in $\bfR$ can be expressed as the \textit{countable} union of disjoint open intervals. {\color{red} ...}
        \end{proof}
