    ...
    \end{proposition}
        \begin{proof}
            ...
            \noindent (6) First note that:
                ...
                \end{split}
                \end{equation*}
            We must now show that $\frac{n}{\gcd(k,n)}$ is the smallest such power of $a^k$ such that $\left( a^k \right)^{\frac{n}{\gcd(k,n)}} = e$. Let $m \in \bfN$ such that $(a^k)^m = a^{km} = e$. Since the order of $a$ is $n$, we have $\div{n}{km}$. Therefore we also have:
                \begin{equation*}
                \begin{split}
                    \frac{n}{\gcd(k,n)} \mid \frac{km}{\gcd{(k,n)}}
                \end{split}
                \end{equation*}
