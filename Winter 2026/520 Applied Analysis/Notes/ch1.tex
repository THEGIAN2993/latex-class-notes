...
\begin{theorem}\label{thm:fd-closed-bdd-implies-cpt}
...
\begin{corollary}[Neumann's Theorem]
    Let $X$ be a Banach space and $T \in \cL(X)$. If $\lnorm T \rnorm < 1$, then $I - T$ is invertible, and $(I - T)^{-1} = \sum_{n = 0}^\infty T^n$.
\end{corollary}
    {\color{red} \begin{proof}
...
\begin{exercise}
Let $Y$ be a linear subspace in a normed linear space $X$. For a fixed $x \in X$, prove that:
    \begin{equation*}
    \begin{split}
        \dist(x,Y) = \sup \{ \varphi(x) \mid \varphi \in X^\ast,\h5 \restr{\varphi}{Y} \equiv 0,\h5 \lnorm \varphi \rnorm = 1\}. 
    \end{split}
    \end{equation*}
\end{exercise}
    \begin{proof}
        Let $\varphi \in X^\ast$ such that $\restr{\varphi}{Y} = 0$ and $\lnorm \varphi \rnorm = 1$. Observe that:
            \begin{equation*}
            \begin{split}
                \varphi(x) 
                & = \varphi(x) - \varphi(y) \\
                & = \varphi(x - y) \\
                & \leq \lnorm \varphi \rnorm \lnorm x-y \rnorm \\
                & = \lnorm x-y \rnorm.
            \end{split}
            \end{equation*}
        Taking the infimum over all $y \in Y$ gives $\varphi(x) \leq \dist(x,Y)$. Taking the supremum over all $\varphi \in X^\ast$ such that $\restr{\varphi}{Y} = 0$ and $\lnorm \varphi \rnorm = 1$ gives $\sup \{ \varphi(x) \mid \varphi \in X^\ast, \restr{\varphi}{Y} = 0, \lnorm \varphi \rnorm = 1\} \leq \dist(x,Y)$.

        For the converse direction, we proceed by cases on $\dist(x,Y)$. If $\dist(x,Y) = 0$, then by the above inequality we have:
            \begin{equation*}
            \begin{split}
                0 
                & = \dist(x,Y) \\
                & \geq \sup \{ \varphi(x) \mid \varphi \in X^\ast, \restr{\varphi}{Y} = 0, \lnorm \varphi \rnorm = 1\} \\
                & \geq 0.
            \end{split}
            \end{equation*}
        Hence $\dist(x,Y) = \sup \{ \varphi(x) \mid \varphi \in X^\ast, \restr{\varphi}{Y} = 0, \lnorm \varphi \rnorm = 1\}$. Now suppose $\dist(x,Y) > 0$. By Corollary~\ref{cor:hb-existence-of-lin-funct} there exists a linear functional $\psi \in X^\ast$ such that $\psi(x) = 1$, $\restr{\psi}{Y} = 0$, and $\lnorm \psi \rnorm = \frac{1}{\dist(x,Y)}$. Define a linear functional $\varPhi:X \rightarrow \bfF$ by $\varPhi(x_0) = \dist(x_0,Y) \psi(x_0)$. Note that $\restr{\varPhi}{Y} = 0$ and: 
            \begin{equation*}
            \begin{split}
                \lnorm  \rnorm
            \end{split}
            \end{equation*}
            $\lnorm \varPhi \rnorm = \dist()$
    \end{proof}


\section{\h1$\ell^p$ Spaces}
\begin{definition}
    \phantom{a}
        \begin{enumerate}[label = (\arabic*),itemsep=1pt,topsep=3pt]
            \item Let $1 \leq p < \infty$. We define $\ell^p := \{x : \bfN \rightarrow \bfF \mid \sum_{j = 1}^\infty |x_j|^p < \infty\}$. Likewise, define $\lnorm \cdot \rnorm_{\ell^p}:\ell^p \rightarrow \bfF$ by $\lnorm x \rnorm_{\ell^p} = \left( \sum_{j = 1}^\infty |x_j|^p \right)^\frac{1}{p}$.
            
            \item Let $p = \infty$. We define $\ell^\infty := \{x:\bfN \rightarrow \bfF \mid \sup_{n \in \bfN}|x_n| < \infty\}$. Likewise, define $\lnorm \cdot \rnorm_{\ell^\infty}:\ell^\infty \rightarrow \bfF$ by $\lnorm x \rnorm_{\ell^\infty} = \sup_{n \in \bfN}|x_n|$.
        \end{enumerate}
\end{definition}

\begin{lemma}\label{lemma:secant}
    Let $a,b \geq 0$ and $0 \leq t \leq 1$. Then $((1-t)a + tb)^p \leq (1-t)a^p + tb^p$.
\end{lemma}
    {\color{red} \begin{proof}
        
    \end{proof}}

\begin{lemma}\label{lemma:p-thing}
    Let $a,b \geq 0$ and $0 \leq t \leq 1$. Then $(a+b)^p \leq 2^{p-1}(a^p +b^p)$.
\end{lemma}
    {\color{red} \begin{proof}
...
\begin{lemma}[Minkowski's Inequality]\label{lemma:minkowski}
    Let $1 \leq p \leq \infty$. For any $x,y \in \ell^p$, we have $\lnorm x + y \rnorm_{\ell^p} \leq \lnorm x \rnorm_{\ell^p} + \lnorm y \rnorm_{\ell^p}$.
\end{lemma}

\begin{theorem}
    For any $1 \leq p < \infty$, $(\ell^p,\lnorm \cdot \rnorm_{\ell^p})$ is a Banach space.
\end{theorem}
    \begin{proof}
        Lemma~\ref{lemma:secant} establishes that $\ell^p$ is a vector space. Lemma~\ref{lemma:minkowski} established that $\lnorm \cdot \rnorm_{\ell^p}$ satisfies the triangle inequality; the remaining norm axioms are routine to check. Hence $(\ell^p,\lnorm \cdot \rnorm_{\ell^p})$ is a normed linear space.

        Let $\{f_n\}_{n = 1}^\infty \subseteq \ell^p$ be a Cauchy sequence. For any $j,n,m \in \bfN$, we have:
            \begin{equation*}
            \begin{split}
                |f_n(j) - f_m(j)| 
                & \leq \left( |f_n(j) - f_m(j)|^p \right)^\frac{1}{p} \\
                & \leq \left( \sum_{k = 1}^\infty |f_n(k) - f_m(k) |^p\right)^\frac{1}{p} \\
                & = \lnorm f_n - f_m \rnorm_{\ell^p}.
            \end{split}
            \end{equation*}
        Hence $\{f_n(j)\}_{n = 1}^\infty \subseteq \bfF$ is Cauchy. Since $\bfF$ is complete, the limit of this sequence exists. Define $f(j) := \lim_{n \rightarrow \infty}f_n(j)$. We will show that $f \in \ell^p$ and that $f_n \rightarrow f$ in $\ell^p$.

        Find $N \in \bfN$ sufficiently large such that for all $n \geq N$, we have $\lnorm f_n - f_N \rnorm_{\ell^p} \leq 1$. Taking $p^\text{th}$ powers on both sides gives the following statement: for all $n \geq N$ we have $\sum_{j = 1}^\infty |f_n(j) - f_N(j)|^p \leq 1$. Now fix $J \in \bfN$. For all $n \geq N$ we have $\sum_{j = 1}^J |f_n(j) - f_N(j)|^p \leq 1$. Take $n \rightarrow \infty$. Then we have $\sum_{j = 1}^J |f(j) - f_N(j)|^p \leq 1$. In particular, for all $J \in \bfN$ we have:
            \begin{equation*}
            \begin{split}
                \sum_{j = 1}^J |f(j)|^p 
                & = \sum_{j = 1}^J |f(j) - f_N(j) + f_N(j)|^p \\
                & \leq \sum_{j = 1}^J 2^{p-1} \left( |f(j) - f_N(j)|^p + |f_N(j)|^p \right) \h9\text{\tiny Lemma~\ref{lemma:p-thing}} \\
                & = 2^{p-1} \left( \sum_{j = 1}^J |f(j) - f_N(j)|^p + \sum_{j = 1}^J |f_N(j)|^p \right) \\
                & \leq 2^{p-1}(1 + \lnorm f_N \rnorm_{\ell^p}^p) \\
                & < \infty.
            \end{split}
            \end{equation*}
        By the Monotone Convergence Theorem, $\sum_{j = 1}^\infty |f(j)|^p < \infty$. Hence $f \in \ell^p$.

        Now let $\epsilon > 0$. Choose $N \in \bfN$ such that $\lnorm f_n - f_N \rnorm_{\ell^p} \leq \epsilon$. Then for all $n \geq N$ and for all $j \in \bfN$, we have $\sum_{j = 1}^J |f_n(j) - f_N(j)|^p \leq \epsilon^p$. Take $n \rightarrow \infty$. Then for all $J \in \bfN$ we have $\sum_{j = 1}^J |f(j) - f_N(j)|^p \leq \epsilon^p$. Using the Monotone Convergence Theorem, take $J \rightarrow \infty$. Then $\sum_{j = 1}^\infty |f(j) - f_N()$ Then $\sum_{j = 1}^\infty |f(j) - f_N(j)|^p \leq \epsilon^p$. Therefore $\lnorm f - f_N \rnorm_{\ell^p} \leq \epsilon$.
    \end{proof}

\begin{definition}
    The subspace of $\ell^\infty$ consisting of all convergent sequences is denoted $c := \{x \in \ell^\infty \mid x \text{ converges}\}$. The subspace of $c$ consisting of all sequences which converge to zero is denoted $c_0 := \{x \in c \mid x \text{ converges to 0}\}$. The subspace of $\ell^\infty$ consisting of all sequences with only finitely many nonzero terms is denoted $c_{00} := \{x \in \ell^\infty \mid \supp(x) < \infty\}$.
\end{definition}

\begin{definition}
    The standard coordinate vector in an infinite dimensional vector space is denoted $e^n := (0,...,0,1,0,...)$; i.e., it is a sequence which is "1" in the $n^\text{th}$ coordinate and "0" elsewhere.
\end{definition}

\begin{proposition}
    Let $y \in \ell^\infty$. Define $\varphi_y:\ell^1 \rightarrow \bfF$ by $\varphi_y(x) = \sum_{k = 1}^\infty x_k y_k$. Then $\varphi \in (\ell^1)^\ast$ and $\lnorm \varphi_y \rnorm_{\ell^1}^\ast = \lnorm y \rnorm_{\ell^\infty}$.
\end{proposition}
...
\end{exercise}
    \begin{proof}
        ...
        \begin{split}
            ...
            & = \sum_{i = 1}^\infty |x_i|| y_i| \\
            & \leq \lnorm y \rnorm_{\ell^\infty}\sum_{i = 1}^\infty |x_i| \\
            & \leq \lnorm y \rnorm_{\ell^\infty}\lnorm x \rnorm_{\ell^1}.
        \end{split}
        \end{equation*}
    Hence:
        \begin{equation*}
        \begin{split}
            |\varphi_y(x)| 
            & = \left|\sum_{i = 1}^\infty x_i y_i  \right| \\
            & \leq \sum_{i = 1}^\infty |x_i y_i| \\
            & \leq \lnorm y \rnorm_{\ell^\infty}\lnorm x \rnorm_{\ell^1}.
        \end{split}
        \end{equation*}
    Dividing by $\lnorm x \rnorm_{\ell^1}$ and taking the supremum over all $x \in \ell^1$, $x \neq 0$ gives the inequality $\lnorm \varphi_y \rnorm_{(\ell^1)^\ast} \leq \lnorm y \rnorm_{\ell^\infty}$. Hence $\varphi_y \in (\ell^1)^\ast$.

    For the converse direction, note that $e_j \in B_{\ell^1}(0,1)$. Hence for any $j \in \bfN$, we have $|y_j| \leq |\varphi_y(e_j)| \leq \lnorm \varphi_y \rnorm_{(\ell^1)^\ast}$. Taking the supremum over all $j \in J$ gives $\lnorm y \rnorm_{\ell^\infty} \leq \lnorm \varphi_y \rnorm_{(\ell^1)^\ast}$. Thus $\lnorm \varphi_y \rnorm_{(\ell^1)^\ast} = \lnorm y \rnorm_{\ell^\infty}$.
    \end{proof}
...
\end{theorem}
    \begin{proof}
        ...
        Conversely, suppose $F:= \{x \mid \sup_{\alpha \in A}\lnorm T_\alpha x \rnorm <\infty \}$ is Category II. Define $F_n := \{x \mid \sup_{\alpha \in A}\lnorm T_\alpha x \rnorm \}$. We will show that $F_n$ is closed in $X$. Note that:
            \begin{equation*}
            \begin{split}
                F_n = \bigcap_{\alpha \in A}\{x \mid \lnorm T_\alpha x \rnorm \leq n\}.
            \end{split}
            \end{equation*}
        Note that $\lnorm \cdot \rnorm$ and $T_\alpha$ are continuous, hence the composition $g_\alpha := \lnorm T_\alpha (\cdot) \rnorm$ is continuous. The above expression then becomes:
            \begin{equation*}
            \begin{split}
                F_n 
                & = \bigcap_{\alpha \in A}\{x \mid \lnorm T_\alpha x \rnorm \leq n\} \\
                & = \bigcap_{\alpha \in A}\{x \mid g_\alpha(x) \leq n\} \\
                & = \bigcap_{\alpha \in A}\{x \mid g_\alpha(x) \in [0,n]\} \\
                & = \bigcap_{\alpha \in A}g_\alpha^{-1}([0,n]).
            \end{split}
            \end{equation*}
        Hence $F_n$ is closed.
         
        Since $F = \bigcup_{n = 1}^\infty F_n$, by Corollary~\ref{cor:bct} there exists some $n \in \bfN$ such that $F_n$ has nonempty interior. Choose $x_0 \in X$ and $r > 0$ such that $\overline{B}(x_0,r) \subset F_n$. Let $x \in X$ such that $\lnorm x \rnorm \leq 1$. Fix $\alpha \in A$. Note that $x_0 + rx \in \overline{B}(x_0,r)$, hence:
            \begin{equation*}
            \begin{split}
...
\begin{example}
{\color{red} jan 29th, bijective but inverse is unbounded}
\end{example}
\begin{example}
{\color{red} bounded but inverse is unbounded}
\end{example}
...
\begin{exercise}
Let $T:X \rightarrow Y$ be a linear map between normed spaces. Prove the assertion made in class: The graph $\{(x,Tx) \mid x \in X\}$ of $T$ is \textit{closed} in $X \times Y$ if and only if:
    \begin{equation*}
    \begin{split}
...
\end{exercise}
    \begin{proof}
        ...
        \item Since $T$ is surjective, continuous, and linear, by the \nameref{thm:open-mapping-thm} $T$ is open.
...
\section{Continuous Dual, Adjoint Maps, and Weak Convergence}

\begin{definition}
Let $X$ and $Y$ be normed linear spaces and $T \in \cL(X,Y)$. The \textui{adjoint of $T$} is a map $T^\ast:Y^\ast \rightarrow X^\ast$ given by $T^\ast(\varphi) = \varphi(Tx)$.
\end{definition}

\begin{proposition}
    Let $X$ and $Y$ be normed linear spaces and $T \in \cL(X,Y)$. Then $T^\ast \in \cL(Y^\ast,X^\ast)$, and $\lnorm T^\ast \rnorm_{\cL(Y^\ast,X^\ast)} = \lnorm T \rnorm_{\cL(X,Y)}$.
\end{proposition}
    {\color{red} \begin{proof}
    \end{proof}}
...
\begin{exercise}\label{exercise:banach-perp-is-closed}
...
\begin{exercise}
    Let $X$ be a normed linear space and $K \subset X$. Show that $(K^\perp)_{\perp}$ is the closure of the linear space $K$ in $X$.
\end{exercise}
    {\color{red} \begin{proof}
        
    \end{proof}}

\section{\h1$L^p$ Spaces}
...
\end{exercise}
    \begin{proof}
        ...
        Note that:
            \begin{equation*}
            \begin{split}
                \int_0^1 x^{-r}dx = -\frac{1}{(r-1)x^{r-1}}\h1\bigg\rvert_{0}^1
            \end{split}
            \end{equation*}
        The right-hand side of this equation can only be evaluated if $r-1 > 0$. Hence $\frac{1}{|x|^r} \in L^1([0,1])$ provided $r>1$. Similarly, note that:
            ...
            \begin{split}
                \int_0^1 (x^{-r})^p dx
                & = \int_0^1 x^{-rp}dx \\
                & = -\frac{1}{(rp-1)x^{rp-1}}\h1\bigg\rvert_{0}^1 \\
            \end{split}
            \end{equation*}
